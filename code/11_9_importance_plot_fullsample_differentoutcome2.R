...
data = data[,c("INC_decile", "nowtp_taxes_weak", "concern", "cc_env_problem", "priorization_env", "knowledge",
...
data$nowtp_taxes_weak <- as.factor(data$nowtp_taxes_weak)
...
set.seed(2022)
forest <- rfsrc(nowtp_taxes_weak ~ ., data = as.data.frame(data), ntree = 1000, nodesize = 1, mtry=6, nsplit=1)
print(forest)
...
write_xlsx(vimpCI, "data\\fullsample_differentoutcome2_vimpCI.xlsx")




# prepare data set
new <- read_excel("data\\fullsample_differentoutcome2_vimpCI.xlsx")
...
#add direction 
direction_data <- read_excel("data\\effect_direction_fullsample_differentoutcome2.xlsx")
...
ggsave(plot = gg, width = 11, height = 11, device = png, dpi = 600, 
       filename = "importance_plot_fullsample_differentoutcome2.png", path = "output\\")
